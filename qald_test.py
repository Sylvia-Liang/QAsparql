...
class Orchestrator:
    ...
    def sort_query(self, linker, kb, parser, qapair, question_type_classifier, force_gold=True):
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            if correct:
                # here the precision and recall is calculated based on the number of correct answer returned
                for si in s_ind:
...
if __name__ == "__main__":
    ...
    output_file = 'qaldanswer_output'
    linker = Earl(path="data/QALD/entity_qald.json")
